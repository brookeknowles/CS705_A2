...
from z3 import And, Not, Xor, Bool, Or, sat, Solver
...
def implementation(Si, Ci, A, B, C, s):
    u, v, w = Bool('u'), Bool('v'), Bool('w')
    s.add(u == Or(And(A, Not(B)), And(Not(A), B)))
    s.add(v == And(u, C))
    s.add(w == And(A, B))
    s.add(Si == Or(And(u, Not(C)), And(Not(u), C)))
    s.add(Ci == Or(w, v))


def main():
    A, B, Cin = Bool('A'), Bool('B'), Bool('Cin')
    Sf, Cf = Bool('Sf'), Bool('Cf')
    s = Solver()
